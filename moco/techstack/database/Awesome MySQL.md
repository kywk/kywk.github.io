---
title: Awesome MySQL
description: MySQL 相關資源與工具整理
tags:
  - Database
  - SQL/MySQL
image: >
  https://lh3.googleusercontent.com/pw/AL9nZEUA9Ifvd5Z8SXDWkeVB6AC4MPGwnXaL6kBXNPoXwOQQ2jOcZ1Jw_0p8TKK8C3ZX0e67_FOY15eDrm7aaXSQJcKtoUzC80SAQEHsaBy6qS2AqNNs5VUFNXBKm439y_1wkvmDl-PnL8ReojnIumNlEvOXBg=w800-no?authuser=0
sidebar_position: 0
hide_table_of_contents: false
date_created: 2025-01-13T00:00:00.000Z
date_updated: 2025-01-13T00:00:00.000Z
---

# Awesome MySQL Resources

MySQL 相關資源、工具與最佳實踐整理。

## 官方資源

- [MySQL Documentation](https://dev.mysql.com/doc/)
- [MySQL Downloads](https://dev.mysql.com/downloads/)
- [MySQL Workbench](https://dev.mysql.com/downloads/workbench/)

## 安裝與設定

- [[MySQL Setup]] - MySQL 基本安裝與設定
- [[MySQL Ubuntu Installation]] - Ubuntu 系統安裝指南

## 管理工具

- [[Awesome Database GUI Client]] - GUI 管理工具比較
- **MySQL Workbench**: 官方 GUI 工具
- **phpMyAdmin**: Web 介面管理
- **Sequel Pro**: macOS 專用客戶端

## 疑難排解

### 密碼重設
- [MySQL 忘記 root 密碼強制進入並重設密碼 - Tsung's Blog](https://blog.longwin.com.tw/2025/01/mysql-forget-reset-root-password-linux-2025/)

### 常見問題
- 連線問題診斷
- 權限設定
- 效能調校

## 替代方案

- **MariaDB**: MySQL 相容的開源分支
- **Percona Server**: 高效能 MySQL 分支
- **AliSQL**: 阿里巴巴維護的 MySQL 分支

## See Also

- [MariaDB Foundation - MariaDB.org](https://mariadb.org)
- [alibaba/AliSQL: AliSQL is a MySQL branch originated from Alibaba Group](https://github.com/alibaba/AliSQL)
- [Percona Server for MySQL](https://www.percona.com/mysql/software)