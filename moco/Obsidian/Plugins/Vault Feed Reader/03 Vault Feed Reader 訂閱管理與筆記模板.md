...
Vault Feed Reader 把訂閱清單、閱讀狀態、我的最愛／稍候閱讀與筆記留在 vault，讓日常閱讀能銜接個人的備份與筆記整理方式。本文說明資料用途、存放位置與設定方式。
...
訂閱 YAML、每來源閱讀狀態、我的最愛與稍候閱讀清單、來源 ID 對照與保存筆記都預設位於此根目錄下；變更後，未來的保存與狀態寫入會改到新位置。
...
| 我的最愛清單 | `Feed Reader/state/favorites.json` | 珍藏文章列表與離線全文快照 |
| 稍候閱讀清單 | `Feed Reader/state/read-later.json` | 待讀文章列表與離線全文快照 |
| 保存筆記 | `Feed Reader/Articles/` | 精選文章與人工註記，可調整路徑 |
| 文章快取 | 本機 IndexedDB | 日常瀏覽使用，可淘汰重建 |

備份時應保留 vault 裡的訂閱、狀態、收藏清單與筆記。其中「我的最愛」與「稍候閱讀」以 JSON 格式儲存於 Vault 內，同時內嵌文章 HTML 全文與中繼資料，即使打包 Vault 帶到另一台電腦，或本機 IndexedDB 快取被清理，也能無縫保留清單與離線內容。插件設定使用 Obsidian 的插件資料儲存；若也要保留模板等偏好，備份策略需涵蓋對應插件設定。AI 提示詞與全文辨識規則也在插件設定中；Agent 路徑、參數、預設 Agent 與偵測結果則存在此裝置、此 vault 專用的 localStorage。換裝置後要重新安裝／登入 CLI 並檢查本機 Agent 設定。
